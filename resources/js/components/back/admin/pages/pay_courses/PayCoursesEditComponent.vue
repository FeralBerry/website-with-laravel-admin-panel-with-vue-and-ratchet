            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form id="pay_courses_edit" method="post" :action="'/admin/pay_courses/edit/'+this.data.pay_courses[0].id" enctype="multipart/form-data">
                                <template v-for="item in this.data.pay_courses">
                                    <input type="hidden" name="_token" :value="csrf">
                                    <span class="bmd-form-group bmd-form-group-lg">
                                        Название
                                        <input class="form-control form-control-lg" name="title" id="pay_courses_title" type="text" :value="item.title" placeholder="Тема урока">
                                    </span>
                                    <br>
                                    Привязка к какому курсу
                                    <br>
                                    <div class="row col-md-12" id="pay_courses_name_checked" >
                                        <template v-for="name in this.data.free_courses_name">
                                            <div class="col-md-3 custom-control custom-radio">
                                                <template v-if="name.id == item.pay_courses_name_id">
                                                    <input checked class="custom-control-input custom-control-input-danger custom-control-input-outline" name="pay_courses_name" :id="'pay_courses_name_'+name.id" :value="name.id" type="radio">
                                                    <label :for="'free_courses_name_'+name.id" class="custom-control-label"> {{ name.title }}</label>
                                                </template>
                                                <template v-if="name.id !== item.pay_courses_name_id">
                                                    <input class="custom-control-input custom-control-input-danger custom-control-input-outline" name="pay_courses_name" :id="'pay_courses_name_'+name.id" :value="name.id" type="radio">
                                                    <label :for="'free_courses_name_'+name.id" class="custom-control-label"> {{ name.title }}</label>
                                                </template>
                                            </div>
                                        </template>
                                    ...
                                    <input type="file" name="last_lesson" id="last_lesson">
                                    <input type="hidden" name="old_last_lesson" id="old_last_lesson" :value="item.last_lesson">
                                    <div class="row col-md-12" id="pay_courses_type">
...
<script>
    export default {
        ...
        created() {
            ...
            setTimeout(function (){
                $('#pay_courses_type').html('<div class="col-md-3 custom-control custom-radio">' +
